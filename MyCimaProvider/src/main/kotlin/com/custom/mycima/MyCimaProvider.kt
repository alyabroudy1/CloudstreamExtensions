...
class MyCima : MainAPI() {
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        return if (isMovie) {
            ...
            val season =
                    doc.select("div.List--Seasons--Episodes a.selected").text().getIntFromText() ?: 1
            ...
            if (seasons.isNotEmpty()) {
                seasons.amap { surl ->
                    if (surl.contains("%d9%85%d8%af%d8%a8%d9%84%d8%ac")) return@amap
    ...
    override suspend fun loadLinks(
        ...
        doc.select(".WatchServersList > ul > li").amap {
        ...
        doc.select("ul.List--Download--Wecima--Single:nth-child(2) li").amap {
            it.select("a").map { linkElement ->
                ...
                callback.invoke(
                        newExtractorLink(
                            ...
                            this.quality = linkElement.select("resolution").text().getIntFromText() ?: 0
                        }
                )
            }
        }
